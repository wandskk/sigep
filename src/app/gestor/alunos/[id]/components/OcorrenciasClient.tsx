...
export function OcorrenciasClient({ alunoId, ocorrenciasIniciais }: OcorrenciasClientProps) {
  ...
  const editarOcorrencia = async (data: OcorrenciaFormData) => {
    ...
    try {
      ...
      const ocorrenciaAtualizada = await response.json();
      setOcorrencias(prev => prev.map(oc => 
        oc.id === ocorrenciaEditando.id ? ocorrenciaAtualizada : oc
      ));
  ...
  const estatisticas = {
    [TipoOcorrencia.ADVERTENCIA]: ocorrencias.filter(o => o.tipo === TipoOcorrencia.ADVERTENCIA).length,
    [TipoOcorrencia.ELOGIO]: ocorrencias.filter(o => o.tipo === TipoOcorrencia.ELOGIO).length,
    [TipoOcorrencia.COMUNICADO]: ocorrencias.filter(o => o.tipo === TipoOcorrencia.COMUNICADO).length,
    [TipoOcorrencia.OUTRO]: ocorrencias.filter(o => o.tipo === TipoOcorrencia.OUTRO).length,
  };

  const getCardStyle = (tipo: TipoOcorrencia) => {
    switch (tipo) {
      case TipoOcorrencia.ADVERTENCIA:
        return "bg-gradient-to-br from-red-50 to-red-100 border-red-200 text-red-700 hover:shadow-md";
      case TipoOcorrencia.ELOGIO:
        return "bg-gradient-to-br from-green-50 to-green-100 border-green-200 text-green-700 hover:shadow-md";
      case TipoOcorrencia.COMUNICADO:
        return "bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200 text-blue-700 hover:shadow-md";
      default:
        return "bg-gradient-to-br from-gray-50 to-gray-100 border-gray-200 text-gray-700 hover:shadow-md";
    }
  };
